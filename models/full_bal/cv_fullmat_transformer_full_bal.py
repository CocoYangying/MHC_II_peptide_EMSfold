#!/usr/bin/env python
"""
cv_fullmat_transformer_v16.py
=========================
5-fold cross-validation training of the full-matrix balance CNN+Transformer model on MHC-peptide
...
class H5SeqDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if "s_z" in self.feature_keys:
            s_z = grp["s_z"][()].astype(np.float32)
            bad_mask = ~np.isfinite(s_z)
            if bad_mask.any():
                s_z = np.nan_to_num(s_z, nan=0.0, posinf=0.0, neginf=0.0, copy=False)
            features["s_z"] = s_z
            
        if "pae" in self.feature_keys:
            pae = grp["pae"][()].astype(np.float32)
            pae_max = np.max(pae) if np.max(pae) > 0 else 1.0
            pae = 1.0 - (pae / pae_max)
            bad_mask = ~np.isfinite(pae)
            if bad_mask.any():
                pae = np.nan_to_num(pae, nan=0.0, posinf=0.0, neginf=0.0, copy=False)
            features["pae"] = pae
            
        if "contact" in self.feature_keys:
            contact = grp["contact"][()].astype(np.float32)
            bad_mask = ~np.isfinite(contact)
            if bad_mask.any():
                contact = np.nan_to_num(contact, nan=0.0, posinf=0.0, neginf=0.0, copy=False)
            features["contact"] = contact

        if self.max_len:
            for key in features:
                if features[key].shape[0] > self.max_len:
                    features[key] = features[key][:self.max_len]
...
def collate_fn(batch, max_len=None):
    ...
    max_len_in_batch = 0
    for b in batch:
        for feat in b["features"].values():
            max_len_in_batch = max(max_len_in_batch, feat.shape[0])
    ...
    for key in batch[0]["features"].keys():
        feat_shape = batch[0]["features"][key].shape
        feat_dim = feat_shape[1] if len(feat_shape) > 1 else 1
        feats_dict[key] = torch.zeros(B, max_len_in_batch, feat_dim, dtype=torch.float32)
    
    mask = torch.zeros(B, max_len_in_batch, dtype=torch.bool)
    
    for i, b in enumerate(batch):
        L_i = min(max_len_in_batch, max([feat.shape[0] for feat in b["features"].values()]))
        
        for key, arr in b["features"].items():
            arr_tensor = torch.from_numpy(arr).float()
            if len(arr_tensor.shape) == 1:
                arr_tensor = arr_tensor.unsqueeze(-1)
                
            if arr_tensor.shape[0] < L_i:
                pad_size = L_i - arr_tensor.shape[0]
                arr_tensor = F.pad(arr_tensor, (0, 0, 0, pad_size))
            elif arr_tensor.shape[0] > L_i:
                arr_tensor = arr_tensor[:L_i]
                
            feats_dict[key][i, :L_i] = arr_tensor
...
class FeatureProjector(nn.Module):
    def __init__(self, feature_dims, target_dim=64):
        super().__init__()
        self.feature_dims = feature_dims
        self.target_dim = target_dim
        
        self.proj_layers = nn.ModuleDict()
        if "s_z" in feature_dims:
            self.proj_layers["s_z"] = nn.Linear(feature_dims["s_z"], target_dim)
        if "pae" in feature_dims:
            self.proj_layers["pae"] = nn.Linear(feature_dims["pae"], target_dim)
        if "contact" in feature_dims:
            self.proj_layers["contact"] = nn.Linear(feature_dims["contact"], target_dim)
        
        num_features = len(feature_dims)
        self.attention = nn.Sequential(
            nn.Linear(target_dim * num_features, target_dim),
            nn.GELU(),
            nn.Linear(target_dim, num_features),
            nn.Softmax(dim=-1)
        )
        
        self.norm = StableLayerNorm(target_dim)
        self.activation = nn.GELU()
        
    def forward(self, features_dict):
        projected_features = []
        for feature_name in self.feature_dims.keys():
            if feature_name in features_dict and feature_name in self.proj_layers:
                proj_feature = self.proj_layers[feature_name](features_dict[feature_name])
                projected_features.append(proj_feature)
        
        if len(projected_features) == 0:
            example_feat = next(iter(features_dict.values()))
            B, L = example_feat.shape[0], example_feat.shape[1]
            return torch.zeros(B, L, self.target_dim, device=example_feat.device)
        
        combined_features = torch.cat(projected_features, dim=-1)
        attention_weights = self.attention(combined_features)
        
        weighted_combined = torch.zeros_like(projected_features[0])
        for i, proj_feature in enumerate(projected_features):
            weight = attention_weights[..., i].unsqueeze(-1)
            weighted_combined += proj_feature * weight
        
        return self.activation(self.norm(weighted_combined))


class PairTransformer(nn.Module):
    def __init__(self, feature_dims, d_model=64, nhead=4, nlayers=2, dropout=0.2, use_flash=True, 
                 cnn_channels=[64, 128], kernel_sizes=[5, 3], pool_sizes=[2, 2]):
        super().__init__()
        ...
        '''
        self.feature_projector = FeatureProjector(
            s_z_dim=feature_dims.get("s_z", 0),
            pae_dim=feature_dims.get("pae", 0),
            contact_dim=feature_dims.get("contact", 0),
            target_dim=d_model
        )
        '''
        ...
        if use_flash:
            self.encoder = nn.ModuleList([
                FlashTransformerEncoderLayer(d_model, nhead, d_model*2, dropout)
        ...
        self.cls_head = nn.Sequential(
            ...
            nn.Dropout(0.3),
            StableLinear(d_model, d_model//2),
            nn.GELU(),
            nn.Dropout(0.2),
            StableLinear(d_model//2, 1)
        )
    ...
    def forward(self, features_dict, mask):
        x = self.feature_projector(features_dict)
...
# Loss / Metrics
# ---------------------------------------------------------------------
def compute_loss(logits, pred_aff, labels, affin, cls_weight=1.0, reg_weight=0.1):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            for key in batch_features:
                batch_features[key] = batch_features[key].to(device, non_blocking=True)
            batch_mask = batch["mask"].to(device, non_blocking=True)
            
            with autocast(enabled=args.use_amp if args else False):
                logits, aff_pred = model(batch_features, batch_mask)
...
def train_fold(fold, args, device):
    ...
    if args.use_fsdp:
        ...
        val_sampler = torch.utils.data.distributed.DistributedSampler(
            val_ds, shuffle=False, drop_last=True
        )
    else:
        train_sampler = None
        val_sampler = None
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            with autocast(enabled=args.use_amp):
                ...
                logits, pred_aff = model(batch_features, batch_mask)
                
                if not torch.all(torch.isfinite(logits)):
                    logger.warning(f"NaN/Inf logits predicted, skip batch {batch_idx}")
        ...
        if rank == 0:
            ...
            current_score = val_auc + 0.1*rmse_norm
...
def main():
    ...
    parser.add_argument("--cnn_channels", nargs='+', type=int, default=[64, 128], help="CNN channel sizes")
    parser.add_argument("--kernel_sizes", nargs='+', type=int, default=[5, 3], help="CNN kernel sizes")
    parser.add_argument("--pool_sizes", nargs='+', type=int, default=[2, 2], help="CNN pool sizes")
