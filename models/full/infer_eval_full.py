...
# -----------------------------------------------------------------------------
from cv_fullmat_transformer_full import (
    H5SeqDataset,
    PairTransformer,
...
from cv_inter_transformer import (
    ...
    InterMLP,
    ResidualBlock,
    StableLinear as InterStableLinear,
    StableLayerNorm as InterStableLayerNorm,
    ...
    with torch.no_grad():
        for batch in loader:
            if mode == "fullmat":
                ...
                logits, pred_aff = model(feat, mask)
            else:
                feat = batch["feat"].to(device)
